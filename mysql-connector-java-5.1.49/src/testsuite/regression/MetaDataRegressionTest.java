...
public class MetaDataRegressionTest extends BaseTestCase {
    ...
    public void testReservedWords() throws Exception {
        ...
        final String mysqlKeywords = "ACCESSIBLE,ANALYZE,ARRAY,ASENSITIVE,BEFORE,BIGINT,BINARY,BLOB,CALL,CHANGE,CONDITION,CUBE,CUME_DIST,DATABASE,DATABASES,"
                + "DAY_HOUR,DAY_MICROSECOND,DAY_MINUTE,DAY_SECOND,DELAYED,DENSE_RANK,DETERMINISTIC,DISTINCTROW,DIV,DUAL,EACH,ELSEIF,EMPTY,ENCLOSED,ESCAPED,"
                + "EXIT,EXPLAIN,FIRST_VALUE,FLOAT4,FLOAT8,FORCE,FULLTEXT,FUNCTION,GENERATED,GROUPING,GROUPS,HIGH_PRIORITY,HOUR_MICROSECOND,HOUR_MINUTE,"
                + "HOUR_SECOND,IF,IGNORE,INDEX,INFILE,INOUT,INT1,INT2,INT3,INT4,INT8,IO_AFTER_GTIDS,IO_BEFORE_GTIDS,ITERATE,JSON_TABLE,KEYS,KILL,LAG,"
                + "LAST_VALUE,LATERAL,LEAD,LEAVE,LIMIT,LINEAR,LINES,LOAD,LOCALTIME,LOCALTIMESTAMP,LOCK,LONG,LONGBLOB,LONGTEXT,LOOP,LOW_PRIORITY,MASTER_BIND,"
                + "MASTER_SSL_VERIFY_SERVER_CERT,MAXVALUE,MEDIUMBLOB,MEDIUMINT,MEDIUMTEXT,MEMBER,MIDDLEINT,MINUTE_MICROSECOND,MINUTE_SECOND,MOD,MODIFIES,"
                + "NO_WRITE_TO_BINLOG,NTH_VALUE,NTILE,OPTIMIZE,OPTIMIZER_COSTS,OPTIONALLY,OUT,OUTFILE,OVER,PARTITION,PERCENT_RANK,PURGE,RANGE,RANK,READS,"
                + "READ_WRITE,RECURSIVE,REGEXP,RELEASE,RENAME,REPEAT,REPLACE,REQUIRE,RESIGNAL,RETURN,RLIKE,ROW,ROW_NUMBER,SCHEMAS,SECOND_MICROSECOND,"
                + "SENSITIVE,SEPARATOR,SHOW,SIGNAL,SPATIAL,SPECIFIC,SQLEXCEPTION,SQLWARNING,SQL_BIG_RESULT,SQL_CALC_FOUND_ROWS,SQL_SMALL_RESULT,SSL,STARTING,"
                + "STORED,STRAIGHT_JOIN,SYSTEM,TERMINATED,TINYBLOB,TINYINT,TINYTEXT,TRIGGER,UNDO,UNLOCK,UNSIGNED,USE,UTC_DATE,UTC_TIME,UTC_TIMESTAMP,"
                + "VARBINARY,VARCHARACTER,VIRTUAL,WHILE,WINDOW,XOR,YEAR_MONTH,ZEROFILL";
